(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{pollIntervalId=null;lastId=``;topic=``;dataCallbacks=new Set;stateCallbacks=new Set;connect(){if(this.pollIntervalId)return;let e=`smart-glasses-hud-dietrich-v2`,t=window.location.hash.match(/topic=([^&]+)/);t&&t[1]&&(e=t[1]),this.topic=e,console.log(`Starting Ntfy HTTP Poller for topic: ${this.topic}...`),this.notifyState(!0),this.pollIntervalId=setInterval(async()=>{try{let e=this.lastId?`since=${this.lastId}`:`since=2m`,t=`https://ntfy.sh/${this.topic}/json?poll=1&${e}`,n=await fetch(t);if(!n.ok)throw Error(`Network response was not ok`);let r=await n.text();if(!r.trim())return;let i=r.trim().split(`
`);for(let e of i){if(!e)continue;let t=JSON.parse(e);if(t.id&&(this.lastId=t.id),t.event===`message`){console.log(`[Ntfy HTTP] Received:`,t.message);let e=JSON.parse(t.message);this.notifyData(e)}}}catch(e){console.error(`Ntfy Polling Error:`,e)}},2e3)}disconnect(){this.pollIntervalId&&(clearInterval(this.pollIntervalId),this.pollIntervalId=null,this.notifyState(!1))}onData(e){this.dataCallbacks.add(e)}onStateChange(e){this.stateCallbacks.add(e)}notifyData(e){this.dataCallbacks.forEach(t=>t(e))}notifyState(e){this.stateCallbacks.forEach(t=>t(e))}};document.querySelector(`#app`).innerHTML=`
