(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=new class{abortController=null;TOPIC_URL=``;dataCallbacks=new Set;stateCallbacks=new Set;async connect(){if(this.abortController)return;let e=`smart-glasses-hud-dietrich-v3`,t=window.location.hash.match(/topic=([^&]+)/);t&&t[1]&&(e=t[1]),this.TOPIC_URL=`https://ntfy.sh/${e}/json`,console.log(`Connecting to Ntfy HTTPS Stream: ${this.TOPIC_URL}...`),this.abortController=new AbortController;try{let e=await fetch(this.TOPIC_URL,{signal:this.abortController.signal});if(!e.ok)throw Error(`HTTP Error: ${e.status}`);console.log(`Ntfy Stream Connected successfully!`),this.notifyState(!0);let t=e.body?.getReader(),n=new TextDecoder,r=``;for(;t;){let{done:e,value:i}=await t.read();if(e)break;r+=n.decode(i,{stream:!0});let a;for(;(a=r.indexOf(`
`))>=0;){let e=r.slice(0,a).trim();if(r=r.slice(a+1),e)try{let t=JSON.parse(e);if(t.event===`message`){console.log(`[Ntfy Stream] Received:`,t.message);let e=JSON.parse(t.message);this.notifyData(e)}}catch(e){console.error(`Failed to parse Ntfy Stream message:`,e)}}}}catch(e){e.name===`AbortError`?console.log(`Ntfy Stream closed intentionally.`):console.error(`Ntfy Stream Error:`,e)}finally{console.log(`Ntfy Stream Closed. Reconnecting in 3s...`),this.notifyState(!1),this.abortController=null,setTimeout(()=>this.connect(),3e3)}}disconnect(){this.abortController&&(this.abortController.abort(),this.abortController=null,this.notifyState(!1))}onData(e){this.dataCallbacks.add(e)}onStateChange(e){this.stateCallbacks.add(e)}notifyData(e){this.dataCallbacks.forEach(t=>t(e))}notifyState(e){this.stateCallbacks.forEach(t=>t(e))}};document.querySelector(`#app`).innerHTML=`
